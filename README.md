...
| 个人健康周计划 | ✅ | 一周饮食日历、营养目标进度条、编辑计划弹窗（localStorage 持久化） |
| 购物清单生成 | ✅ | 基于当前周计划一键生成食材清单，支持分类分组、勾选已购、复制文字 |
| AI 菜谱生成 | ✅ | DeepSeek AI 根据营养偏好生成菜谱建议 |
| 导航栏全局搜索 | ✅ | 任意页面可展开搜索框，回车直达菜谱探索页 |
| 轻量登录页 | ✅ | 支持用户名密码登录、会话内保持登录、顶部显示用户名与退出 |
| 登录页设计系统对齐 | ✅ | 登录页 UI 使用 shadcn/ui 组件与 CSS 变量 token，完全匹配全站设计风格 |
