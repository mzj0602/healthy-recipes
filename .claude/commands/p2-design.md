...
**强制约束：**
- 如果命令 exit code 非 0 → 立即停止，发送 TG 告警后退出，禁止跳过：
  ```bash
  node /Users/mzj/Desktop/healthy-recipes/scripts/notify-tg.js "❌ P2 中断：{feature-name}
  Codex 设计审查命令执行失败（exit code 非 0），需人工介入。"
  ```
- **必须**将 Codex 完整原始输出追加到 design.md 末尾的 `## Codex Design Review` 章节，禁止省略或总结替代
- Codex 输出少于 3 行 → 视为审查粒度不足，重新调用一次（最多重试 1 次）；仍不足则在章节末注明"审查输出过短，已记录原文"
- 输出包含问题 → 逐条修订 design.md，并追加：
  ```markdown
  ## Codex Design Review
  ### Codex 原始输出
  {完整原始输出，一字不改}
